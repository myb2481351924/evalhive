...
async def m_faithfulness(
    ...
    task = (
        "Break the ANSWER into individual factual claims. Count how many claims are "
        "fully supported by the REFERENCE CONTEXT. Report the count on a line "
        "'SUPPORTED: x/y', then give SCORE = round(5 * x / y). A claim inventing "
        "facts not in the context is unsupported."
    )
    provider = ctx.resolve_judge(a)
    if provider is None:
        return MetricResult(
            metric="faithfulness", score=0.0, passed=False, detail="no judge provider configured"
        )
    prompt = build_prompt(task, case, response, a.rubric)
    jr = await ctx.ask(provider.config.id, prompt)
    if not jr.ok:
        return MetricResult(
            metric="faithfulness", score=0.0, passed=False, detail=f"judge call failed: {jr.error}"
        )
    ...
    score = (
        ratio if ratio is not None else (raw / 5 if raw is not None else (1.0 if verdict else 0.0))
    )
